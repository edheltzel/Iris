...
- Keep `<root>/iris -> current/iris` stable. A marked legacy Spynel installation may upgrade once by migrating exact owned startup registrations and replacing its exact owned launchers without preserving a `spynel` alias; unrelated entries remain untouched. Recognize the legacy module and `spynel` basename only under verified installation-owned paths so `killall` and uninstall can stop an older release, but treat it as an uncoordinated blocker for update/restart. Resolve restart and automatic-startup through the Iris launcher even when `os.Executable` names an older bundle. Retain old bundles and inert interrupted stages; explicit uninstall stops every process using the installation. Do not add speculative process garbage collection.
