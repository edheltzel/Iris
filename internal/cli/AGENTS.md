...
- Keep standalone bootstrap `install-bundle` independent of workspace initialization. Reuse updater ownership to migrate exact legacy startup registrations during publication and route explicit update completion and ordinary restart through the stable installed entry point, preserving primary/job shutdown and init continuation arguments. Authorize proactive standalone checks only in an actual interactive TUI launch without automatic-startup or skip-check flags.
...
- Internal `uninstall-bundles` is workspace-independent. Discover the current global npm root before elevation, retain the original HOME/PATH/user identity across one required native sudo authorization, and coordinate updater removal with startup cleanup for both standalone and npm installations, including exact adopted-root `iris` and `spynel` startup launchers. The public `uninstall.sh` uses the checksummed native bootstrap so cleanup runs outside either installation.
- Internal `cleanup-legacy-npm` is workspace-independent and accepts only an ownership-validated legacy npm root plus an authorized target user. Resolve that user's account home instead of trusting the invoking environment, remove exact startup registrations for its `spynel.js`, then stop only processes whose native executable belongs to that root; npm retains ownership of package removal.

- `iris update` acts on the caller's managed installation without joining a workspace primary; bare update applies and restarts all its instances, `check` only checks. Channel updates select their primary's installation through the shared updater. Register each server/TUI for its entire election lifetime and translate the updater restart signal into ordinary graceful shutdown and exec. Internal launcher coordination commands are workspace-independent. `iris killall` stops verified processes across installations and asks startup ownership to stop matching services without removing future registrations; verified legacy records map only to their exact installation-owned stable `spynel` launcher or npm `spynel.js` registration.
