...
- Internal `cleanup-legacy-npm` is workspace-independent and accepts only an ownership-validated legacy npm root. Remove exact startup registrations for its `spynel.js`, then stop only processes whose native executable belongs to that root; npm retains ownership of package removal.
