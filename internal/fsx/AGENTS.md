...
- Cross-filesystem directory migration copies into a destination sibling, publishes the complete tree atomically, and removes the source only after publication. No-clobber merges add missing entries, accept identical existing entries, fail on conflicts, and remove the source only after a complete merge. Preserve regular-file modes and symlinks, and reject unsupported entry types.
