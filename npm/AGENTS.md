...
- A global install or update removes the legacy unscoped `spynel` package only when a real package directory in the same npm global root has the exact prior package name, repository, and binary metadata. Before npm removes that verified tree, the installed Iris binary removes exact startup registrations for its `spynel.js` and stops only processes owned by that package root. Preserve linked, malformed, and unrelated packages.
