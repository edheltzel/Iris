...
- A global install or update removes the legacy unscoped `spynel` package only when a real package directory in the same npm global root has the exact prior package name, repository, and binary metadata. Preserve linked, malformed, and unrelated packages.
