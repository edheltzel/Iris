...
- Do not run the Go program during ordinary package installation. The sole exception is the internal ownership-validated legacy npm cleanup immediately before removing the exact known prior package.
...
- A global install or update removes the legacy unscoped `spynel` package only when a real package directory in the same npm global root has the exact prior package name, repository, and binary metadata. Before npm removes that verified tree, the installed Iris binary uses the authorized original user identity and account home to remove exact startup registrations for its `spynel.js`, then stops only processes owned by that package root. Preserve linked, malformed, and unrelated packages.
