...
- `install-dev.sh` reuses that disposable build, atomically installs the executable into `$SPYNEL_DEV_BIN_DIR` or the conventional per-user `$HOME/.local/bin`, removes only the former same-directory `spynel` development target, never edits shell profiles, and reports exact PATH setup or shadowing guidance so later terminals can resolve the development build safely.
