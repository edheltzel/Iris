...
iris docs
iris docs goals
iris docs goals page 1
iris docs search review
iris docs search "primary election" page 2 --format json
```
...
`iris instructions [--config PATH]` is a separate read-only workspace inspection command. It reports the five role names, workspace-relative paths, presence, byte counts, and validation errors without displaying saved instruction contents or starting a server or harness.

Static topics describe user commands, workflow contracts, and implementation architecture. They label runtime-only subjects such as jobs and logs and never represent live values. Use `iris status`, `jobs`, `tasks`, `goals`, `logs`, and the actual task/goal documents for current state.
...
```bash
iris send --conversation deploy --stream "Check the release state"
printf '%s\n' 'Review these multiline notes.' 'Keep the answer brief.' | \
  iris send --conversation review --stdin
iris send --conversation review --attach ./trace.log "Diagnose this trace"
iris send --conversation bot --json "Report current work" | jq -c .
```

## Follow up during an active turn

```bash
iris followup [send flags] TEXT
```
...
```bash
iris conversations list [--config PATH] [--limit 1..1000] [--json]
iris conversations show [--config PATH] [--tail 1..1000] [--chars 1..2097152] [--json] CHANNEL CONVERSATION
iris conversations resume [--config PATH] [--json] CHANNEL CONVERSATION
```
...
```bash
branch=$(iris conversations resume telegram TG-42)
iris send --conversation "$branch" "Continue this conversation from the saved context"
```

## Status and framework commands

```bash
iris status [--config PATH] [--conversation NAME] [--json]
iris command [--config PATH] [--conversation NAME] [--json] NAME [ARGUMENTS...]
```
...
iris tasks
iris tasks --limit 50
iris tasks recent --days 14
iris tasks review
iris tasks waiting --detail
iris tasks done
iris tasks failed
iris tasks --json failed
iris goals
iris goals active
iris goals review
iris goals failed
iris goals all --days 30
iris job info 3
iris job message 3 "Prioritize the API regression"
iris job ping 3
iris job kill 3
iris log page 2
iris log search webhook
iris stop --conversation deploy
iris clear --conversation deploy
iris harness codex
iris model gpt-5
iris telegram on
iris whatsapp off
...
```bash
iris task "Fix the failing login test"
iris task --no-review "Correct the README typo and verify the result"
iris task inspect .spynel/tasks/todo/example.md
iris goal "Keep dependencies current"
iris run --once
iris extension list
iris extension install https://github.com/example/spynel-hooks.git
iris extension remove spynel-hooks
```

The shared slash commands have different, conversation-aware behavior. `iris command --conversation NAME task "..."` and its `/task` equivalent combine the user-overridable `.spynel/prompts/create-task.md` directive and the literal request with the ordinary communication prompt; `/goal` uses the matching goal directive. The communication agent checks for duplicates and creates or refines the complete document. These slash paths therefore require the selected harness, while the direct `iris task` and `iris goal` helpers remain suitable for offline scripts.
