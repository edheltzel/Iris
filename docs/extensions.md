...
`extensions/quota` is a companion, copied into a workspace's extensions directory. `watch.sh` is an external poll Iris does not arm: an unchanged checksum of `.spynel/tasks` and `.spynel/goals` Markdown files prints nothing, and a change prints one wake line plus a `quota-axi --no-credential-refresh` snapshot. `task.claimed` writes that snapshot to `.spynel/extensions-state/quota/latest.txt` and adds one `Quota evidence:` progress line to the claimed file. The hook always exits zero and does not cancel the claim, rank a model, or change the semantic heartbeat. `extensions/quota/README.md` documents both commands.
