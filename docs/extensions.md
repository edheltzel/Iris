...
`extensions/no-mistakes` refuses `task.completed` when `.spynel/no-mistakes-mode` is `no-mistakes` or `no-mistakes-prod-only` (the default is `no-mistakes`) and `no-mistakes status` is missing, failing, or has no `gate:` line. `direct-PR` and `local-only` journal a skip and exit zero. `failed`, `cancelled`, and `waiting` outcomes skip. `harness.after` journals the same check and exits zero, because a nonzero exit there replaces the chat response. The pack never runs `no-mistakes axi run`. `extensions/no-mistakes/README.md` documents the mode file and journal.
