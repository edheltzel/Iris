...
`extensions/quota` is an optional heartbeat companion, copied into a workspace's extensions directory as `quota/wake.sh`. Iris runs it before a semantic heartbeat provider turn. A first line of `absorb` skips that turn. Any other result, a missing script, or a script error keeps the provider turn. The script absorbs only when `.spynel/tasks/waiting` has no Markdown file. A waiting task prints `dispatch` and, when installed, a `quota-axi --no-credential-refresh` snapshot for the heartbeat agent. The script does not rank models or send reminders.
